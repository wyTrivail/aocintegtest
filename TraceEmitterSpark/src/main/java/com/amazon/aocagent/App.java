...
public class App {
  ...
  public static void main(String[] args) {
    get("/span0", (req, res) -> {
      makeHttpCall("http://localhost:4567/span1");
      String traceId = TracingContextUtils.getCurrentSpan().getContext().getTraceId().toLowerBase16();
      String xrayTraceId = "#/traces/1-" + traceId.substring(0, 8)
          + "-" + traceId.substring(8);
      return xrayTraceId + "/" + traceId;
    });

    get("/span1", (req, res) -> {
      makeHttpCall("http://localhost:4567/span2");
      return "ok";
    });
