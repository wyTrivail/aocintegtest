...
public enum TestAMI {
  ...
  /**
   * getS3Key returns the s3 key for the ot package base on the AMI,
   * which could be used to construct the downloading link with the s3 domain.
  ...
  public String getDownloadingCommand(String fromUrl, String toLocation) throws BaseException {
    switch (this) {
      case AMAZON_LINUX2:
        return String.format("curl %s -o %s", fromUrl, toLocation);
      default:
