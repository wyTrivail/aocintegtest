...
public class SSMHelper {
    ...
    public static final String UPDATE_SSM_AGENT_DOCUMENT = "AWS-UpdateSSMAgent";
    public static final String RUN_POWER_SHELL_SCRIPT_DOCUMENT = "AWS-RunPowerShellScript";
    ...
    public void updateSsmAgentToLatest(final String instanceId)
        ...
        final Command command = ssmClient.sendCommand(new SendCommandRequest()
                ...
                .withDocumentName(UPDATE_SSM_AGENT_DOCUMENT)
    ...
    public void runShellScriptCommand(final String instanceId, final List<String> commands,
        ...
        if (!"Success".equals(trackCommandStatus(command))){
            throw new RuntimeException("On "+ instanceId +", failed to execute the command: " + commands);
        }
    }

    private String trackCommandStatus(final Command command) throws InterruptedException {
        log.info("Starting command {} for document {}", command.getCommandId(), command.getDocumentName());
        ...
        while (CommandStatus.fromValue(status) == CommandStatus.Pending ||
            ...
            status = ssmClient.listCommands(new ListCommandsRequest().withCommandId(command.getCommandId())).getCommands().get(0).getStatus();
        }
        StringBuffer sb = new StringBuffer();
        try {
            List<CommandPlugin> commandPlugins = ssmClient.listCommandInvocations(
                    new ListCommandInvocationsRequest().withCommandId(command.getCommandId()).withDetails(true)
    ...
    private InstanceInformation getInstanceInformation(final String instanceId) {
        try {
            InstanceInformationFilter instanceInformationFilter = new InstanceInformationFilter().withKey("InstanceIds").withValueSet(instanceId);
            DescribeInstanceInformationRequest describeInstanceInformationRequest = new DescribeInstanceInformationRequest().withInstanceInformationFilterList(Collections
                    .singletonList(instanceInformationFilter));

            final List<InstanceInformation> instances = ssmClient.describeInstanceInformation(describeInstanceInformationRequest).getInstanceInformationList();
