...
public class OTPackageInstaller implements OTInstaller {
  ...
  private void downloadPackage() throws Exception {
    ...
    if (context.getTestingAMI().getOSType().equals(OSType.WINDOWS)) {
      ssmHelper.runShellScriptCommand(context.getInstanceId(), Arrays.asList(downloadingCommand),
              GenericConstants.RUN_POWER_SHELL_SCRIPT_DOCUMENT.getVal());
    } else {
      RetryHelper.retry(
          () -> {
  ...
  private void installPackage() throws Exception {
    ...
    if (context.getTestingAMI().getOSType().equals(OSType.WINDOWS)) {
      ssmHelper.runShellScriptCommand(context.getInstanceId(), Arrays.asList(installingCommand),
              GenericConstants.RUN_POWER_SHELL_SCRIPT_DOCUMENT.getVal());
    } else {
      RetryHelper.retry(
          () -> {
  ...
  private void configureAndStart() throws Exception {
    ...
    if (context.getTestingAMI().getOSType().equals(OSType.WINDOWS)) {
      ...
      ssmHelper.runShellScriptCommand(context.getInstanceId(), Arrays.asList(configuringCommand),
              GenericConstants.RUN_POWER_SHELL_SCRIPT_DOCUMENT.getVal());
    } else {
      String configuringCommand =
              String.format(
    ...
    if (context.getTestingAMI().getOSType().equals(OSType.WINDOWS)) {
      ...
      ssmHelper.runShellScriptCommand(context.getInstanceId(),
              Arrays.asList(startingCommand, disableFirewallCommand),
              GenericConstants.RUN_POWER_SHELL_SCRIPT_DOCUMENT.getVal());
    } else {
      String startingCommand = context.getTestingAMI()
