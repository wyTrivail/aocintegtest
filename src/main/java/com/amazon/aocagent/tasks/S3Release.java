...
/**
 * S3Release will upload the packages[rpm, deb, msi, pkg] into the configured S3 bucket.
 * it will be used for AOC distribution as well as the pre-flight step in Integ-test to simulate the releases in the Testing Stack
 */
public class S3Release implements ITask {
    ...
    public void releasePackagesToS3() throws BaseException {
        ...
        /**
         * upload local packages to s3 with the "latest" key, override the key if it's already existed in the bucket
         */
    ...
    private void validateLocalPackage() throws BaseException {
        for(LocalPackage builtPackage: LocalPackage.values()){
            /**
             * assuming the local directory is
             * os/arch/version/
             */
            String filePath = builtPackage.getFilePath(context.getLocalPackagesDir());
            if(!Files.exists(Paths.get(filePath))){
                throw new BaseException(ExceptionCode.LOCAL_PACKAGE_NOT_EXIST, "local package not exist: " + filePath);
            }
        }
    }
